...
map<string, double> busTimeUpStopCost;
...
vector<string> SplitLine(string stringplit, string strs)
{
	...
	int pos(-1);
	if (strs[strs.size() - 1] == '\r') //去除末尾'\r'
		strs.erase(strs.size() - 1, 1);
	strs += stringplit;
...
//读Cost缓存文件
map<string, double> ReadCostCache(string cachepath)
{
	map<string, double> costcache;
	ifstream fin(cachepath.c_str());
	string str, substri;
	while (getline(fin, str))
	{
		vector<string> thisline = SplitLine(",", str);
		double temp = atof(thisline[1].c_str());
		costcache[thisline[0]] = temp;
	}
	return costcache;
}
...
map<string, double> loadTrainBusTimeUPStop(string basicpath, set<string> targetBusTimeUp)
{
	...
	for (int i = 1009; i < 1025; i++)  //2017年十一放假是1-8号
	{
		// 存放这一天的时间、站点的信息
		...
		/*string printfpath = "data/printf.csv";
		ofstream fou(printfpath.c_str());
		map<string, map<int, int>>::iterator temp;
		for (temp = todayRecords.begin(); temp != todayRecords.end(); ++temp)
		{
			fou << temp->first << ',' << temp->second.begin()->first << ',' << temp->second.begin()->second << endl;
		}*/
		//exit(0);
		...
		for (map<string, map<int, int>>::iterator itbustime = todayRecords.begin(); itbustime != todayRecords.end(); ++itbustime)
		{
			...
			map<int, int> correctTimeStop; //用于存放到站时刻 <时间戳，站>
				...
				/*if (ittime1->second == 2)
			{
				...
				while (ittime2 != correctTimeStop.end())
				{
					if (ittime2->second == ittime1->second + 1)
					{
						...
						string busTimeUpStop = itbustime->first + "-" + strstop;
						if (results.count(busTimeUpStop))
							results[busTimeUpStop].push_back(ittime2->first - ittime1->first); //[211-8-1-7,120]
						else
						{
							vector<int> temp;
	...
	// 最后，取平均数吧！
	/*map<string, double> res; //<211-8-1-10,120>平均时间
	for (map<string, vector<int>>::iterator itres = results.begin(); itres != results.end(); ++itres)
	{
		int tempsum = 0;
		for (vector<int>::iterator ittime = itres->second.begin(); ittime != itres->second.end(); ++ittime)
	...
	return res; //（busid-hour-up-curstation，两站时间)
}

/* datapath预测集文件名, busTimeUpStopCost为（busid-hour-up-curstation，两站时间), respath预测结果保存文件 */
map<string, vector<double>> predPathTimes(string datapath, map<string, double> busTimeUpStopCost, string respath)
{
	...
	while (getline(fin, str))
	{
		...
		int stStop = atoi(thisline[4].c_str()), enStop = atoi(thisline[5].c_str()); //开始站和结束站
		...
		for (int stopi = stStop; stopi <= enStop; ++stopi)
		{
			...
			isis << stopi;
			isis >> strStopi;
			string busTimeUpStop = infokey + "-" + strStopi; //busid-hour-up-curstation
		...
		string thiskey;
		for (int i = 0; i < 6; i++) //组合成10-26,211,902067,9:00:00,7,31
...
int main()
{
	...
	//busTimeUpStopCost = loadTrainBusTimeUPStop(basicpath, targetBusTimeUp);

	string respath = "data/submitted.csv";
	string cachepath = "data/busTimeUpStopCost.csv";
	busTimeUpStopCost = ReadCostCache(cachepath);
	map<string, vector<double>> results = predPathTimes(predPath, busTimeUpStopCost, respath);
	/* predPath预测集文件名, busTimeUpStopCost为（busid-hour-up-curstation，两站时间), respath预测结果保存文件 */
	PrintTime();
	return 0;
}