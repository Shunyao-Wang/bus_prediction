...
for i in range(1001,1032):
    ...
    del [df_temp['O_LINENO_START'],df_temp['O_TERMINALNO_START'],df_temp['O_UP_START'],df_temp['DATE_TIME_START']]
...
#for i in range(50):
    ...
    for j in range(startid, endid+1):
        ...
        df_temp = df_train[df_train.STATION_ID_START == start_real_id]
        df_temp = df_temp[df_temp.STATION_ID_END == end_real_id]
        
        ''' 工作日、周末 '''
        if weekday in range(5):
            df_temp = df_temp[(df_temp.DATE_TIME_END.dt.weekday.between(0,4))]
        else:
            df_temp = df_temp[(df_temp.DATE_TIME_END.dt.weekday.between(5,6))]

        #df_temp = df_temp[(df_temp.DATE_TIME_END.dt.weekday == df_test.loc[i, 'TIME'].weekday())]
        
        
        ''' 动态时间范围 '''
        fromtime = (temp_dt - datetime.timedelta(seconds=2400) + datetime.timedelta(seconds=total)).time()
        totime = (temp_dt + datetime.timedelta(seconds=2400) + datetime.timedelta(seconds=total)).time()
        df_temp = df_temp[(df_temp.DATE_TIME_END.dt.time.between(fromtime,totime))]
        
        
        if len(df_temp.index) != 0:
            total += df_temp['TRAVEL_TIME'].dt.seconds.median()  #中位数
            matched_time += 1
        else:
            total += 120
...
a = str(datetime.datetime.strftime(datetime.datetime.today(),'%Y%m%d_%H%M%S')) + '_' + str(matched_time)
df_result.to_csv('../bus_data/result/{}.csv'.format(a),index=False)
print(a)